...
import { ClickUpPreferences, clickupService } from '@/services/clickup.service';

interface ClickUpPreferencesCardProps {
...
export function ClickUpPreferencesCard({
  preferences: serverPreferences,
  disabled,
}: ClickUpPreferencesCardProps) {
  ...
  const mutation = useMutation({
    mutationFn: (next: ClickUpPreferences) => clickupService.updatePreferences(next),
    onSuccess: (res) => {
      const next = res.data;
      setPreferences(next);
      queryClient.setQueryData(['clickup-status'], (current: unknown) => {
  ...
  const update = (key: keyof ClickUpPreferences, value: boolean) => {
  ...
  return (
    <Card>
      ...
      <CardContent className="space-y-3">
        <ToggleRow
          label="Lists"
          description="Show ClickUp lists and tasks when you select one"
          checked={preferences.showLists}
          disabled={isPending}
          onChange={(value) => update('showLists', value)}
        />
      </CardContent>
    </Card>
  );
