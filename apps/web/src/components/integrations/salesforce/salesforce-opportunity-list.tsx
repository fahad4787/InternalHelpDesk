...
import { CalendarClock, DollarSign, ExternalLink, Percent } from 'lucide-react';
import { Badge } from '@/components/ui/badge';
import { SalesforceOpportunity } from '@/services/salesforce.service';

interface SalesforceOpportunityListProps {
...
function OpportunityCard({
  ...
  const amount = formatCurrency(opportunity.amount);
        ...
        <div className="min-w-0 flex-1">
          <div className="mb-2 flex flex-wrap items-center gap-2">
            ...
            {opportunity.stageName && (
              <Badge variant="info">{opportunity.stageName}</Badge>
            )}
          </div>
            ...
            {opportunity.closeDate && (
              <div className="flex items-center gap-2">
                ...
                <span className="truncate">
                  Closes{' '}
                  {format(new Date(opportunity.closeDate), 'MMM d, yyyy')}
                </span>
              </div>
            )}
...
export function SalesforceOpportunityList({
  opportunities,
}: SalesforceOpportunityListProps) {
  return (
