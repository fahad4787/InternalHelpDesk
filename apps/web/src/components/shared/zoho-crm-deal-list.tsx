...
import { ZohoCrmDeal } from '@/services/zoho-crm.service';

interface ZohoCrmDealListProps {
  deals: ZohoCrmDeal[];
}
...
function DealCard({ deal }: { deal: ZohoCrmDeal }) {
          ...
          <div className="space-y-1.5 text-sm text-muted">
            ...
            {deal.closingDate && (
              <div className="flex items-center gap-2">
                ...
                <span>
                  Closes {format(new Date(deal.closingDate), 'MMM d, yyyy')}
                </span>
              </div>
            )}
...
export function ZohoCrmDealList({ deals }: ZohoCrmDealListProps) {
  return (
