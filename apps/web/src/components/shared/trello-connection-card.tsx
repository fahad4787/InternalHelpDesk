            ...
            <div className="min-w-0 space-y-1">
              ...
              {status?.trelloEmail ? (
                ...
                <p className="text-sm text-muted">
                  Link your Trello account to view boards and cards
                </p>
              )}
            ...
            {authError && (
              <p className="rounded-lg border border-red-200 bg-red-50 px-3 py-2 text-sm text-red-700">
                {authError === 'access_denied'
                  ? 'Trello access was denied. Approve the app when prompted.'
                  : authError === 'missing_token' || authError === 'missing_state'
                    ? 'Trello sign-in was interrupted. Refresh the page and connect again if needed.'
                    : authError}
              </p>
            )}
            {connectError && (
