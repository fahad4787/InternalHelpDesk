...
interface DropboxPreferencesCardProps {
...
export function DropboxPreferencesCard({
  preferences: serverPreferences,
  disabled,
}: DropboxPreferencesCardProps) {
  ...
  const mutation = useMutation({
    mutationFn: (next: DropboxPreferences) =>
      dropboxService.updatePreferences(next),
    onSuccess: (res) => {
      const next = res.data;
      setPreferences(next);
      queryClient.setQueryData(['dropbox-status'], (current: unknown) => {
  ...
  const update = (key: keyof DropboxPreferences, value: boolean) => {
  ...
  return (
    <Card>
      ...
      <CardContent className="space-y-3">
        <ToggleRow
          label="Files"
          description="Show recent files and folders from your Dropbox"
