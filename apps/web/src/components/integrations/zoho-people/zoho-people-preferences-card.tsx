...
interface ZohoPeoplePreferencesCardProps {
...
export function ZohoPeoplePreferencesCard({
  preferences: serverPreferences,
  disabled,
}: ZohoPeoplePreferencesCardProps) {
  ...
  const mutation = useMutation({
    mutationFn: (next: ZohoPeoplePreferences) =>
      zohoPeopleService.updatePreferences(next),
    onSuccess: (res) => {
      const next = res.data;
      setPreferences(next);
      queryClient.setQueryData(['zoho-people-status'], (current: unknown) => {
  ...
  const update = (key: keyof ZohoPeoplePreferences, value: boolean) => {
  ...
  return (
    <Card>
      ...
      <CardContent className="space-y-3">
        <ToggleRow
          label="Employees"
          description="Show employees from Zoho People"
          checked={preferences.showEmployees}
          disabled={isPending}
          onChange={(value) => update('showEmployees', value)}
        />
        <ToggleRow
          label="Leave"
          description="Show leave requests from Zoho People"
          checked={preferences.showLeave}
          disabled={isPending}
          onChange={(value) => update('showLeave', value)}
        />
      </CardContent>
    </Card>
  );
