...
interface OneDrivePreferencesCardProps {
...
export function OneDrivePreferencesCard({
  preferences: serverPreferences,
  disabled,
}: OneDrivePreferencesCardProps) {
  ...
  const mutation = useMutation({
    mutationFn: (next: OneDrivePreferences) =>
      oneDriveService.updatePreferences(next),
    onSuccess: (res) => {
      const next = res.data;
      setPreferences(next);
      queryClient.setQueryData(['onedrive-status'], (current: unknown) => {
  ...
  const update = (key: keyof OneDrivePreferences, value: boolean) => {
  ...
  return (
    <Card>
      ...
      <CardContent className="space-y-3">
        <ToggleRow
          label="Files"
          description="Show recent files and folders from your OneDrive"
