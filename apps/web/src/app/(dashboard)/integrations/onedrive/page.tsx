...
export default function OneDriveIntegrationPage() {
  ...
  const preferences = status?.preferences ?? DEFAULT_ONEDRIVE_PREFERENCES;
  ...
  const connectMutation = useMutation({
    mutationFn: () => oneDriveService.getAuthUrl(),
    onSuccess: (res) => {
  ...
  return (
    <PageContainer
      title="OneDrive"
      description="Files and folders from your linked OneDrive account"
      actions={
    ...
    >
      <div className="space-y-6">
        ...
        {isConnected && <OneDrivePreferencesCard preferences={preferences} />}

        {isConnected && <IntegrationWidgetsSection provider="ONEDRIVE" />}
      </div>
    </PageContainer>
  );
}
