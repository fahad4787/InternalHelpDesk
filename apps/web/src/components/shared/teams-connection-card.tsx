...
import { CheckCircle2, Mail, Unplug, Users } from 'lucide-react';
            ...
            <div className="min-w-0 space-y-1">
              ...
              {status?.tenantName ? (
                ...
                <p className="text-sm text-slate-500">
                  Connect Microsoft Teams for channel access
                </p>
              )}
          ...
          <div className="space-y-2 border-t border-slate-100 bg-white/70 px-5 py-3">
            {status?.mockMode && (
              <p className="rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 text-xs text-amber-800">
                Mock mode is enabled. Set TEAMS_MODE=live and Microsoft Azure app credentials to use live authentication.
              </p>
            )}
            {authError && (
              <p className="rounded-lg border border-red-200 bg-red-50 px-3 py-2 text-sm text-red-700">
                {authError === 'access_denied'
                  ? 'Microsoft access was denied. Approve the app when prompted.'
                  : authError === 'missing_code' || authError === 'missing_state'
                    ? 'Microsoft sign-in was interrupted. Refresh the page and connect again if needed.'
                    : authError}
              </p>
            )}
            {connectError && (
